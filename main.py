...
class User(UserMixin, db.Model):
    ...
    # Adds another parent relationship
    #'comment_author' refers to comment_author property in Comment class
...
class Comment(db.Model):
    ...
    # "users.id" the users refers to the table name of User.
    # 'comments' refers to the comments property in the User class.
    author_id = db.Column(db.Integer, db.ForeignKey('users.id'))
    comment_author = relationship('User', back_populates='comments')
    # Adds another child relationship
...
# initialize gravatar (avatar graphics for users)
...
def admin_only(function):
    ...
    def decorated_function(*args, **kwargs):
        if current_user.id != 1:
            flash("Access denied. You are not the administrator")
...
def register():
    ...
    if form.validate_on_submit():
        if User.query.filter_by(email=form.email.data).first():
            flash("This email has already been registered. Please login instead", 'error')
